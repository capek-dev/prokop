...
test('Git revert sends a literal path and selected root to the encoded workspace endpoint', async () => {
  let capturedUrl = '';
  let capturedInit: RequestInit = {};
  globalThis.fetch = mock(async (url: string | URL | Request, init?: RequestInit) => {
    capturedUrl = String(url);
    capturedInit = init ?? {};
    return Response.json({ path: '[changed].txt' });
  }) as typeof fetch;
  const files = new FilesRestNamespace(new HttpClient({ url: 'https://example.com' }));
  const result = await files.gitRevertModifiedFile('ws/1', '[changed].txt', { root: '/worktree' });
  expect(capturedUrl).toBe('https://example.com/api/workspaces/ws%2F1/git/revert-modified-file');
  expect(capturedInit.method).toBe('POST');
  expect(JSON.parse(capturedInit.body as string)).toEqual({ path: '[changed].txt', root: '/worktree' });
  expect(result).toEqual({ path: '[changed].txt' });
});
