...
describe('capability guards and projections', () => {
  test('registers guards and projections with deterministic ordering across reversed plugin input', async () => {
    const buildPlugins = (): CapekPlugin<unknown>[] => [
      makePlugin({
        id: 'z-plugin',
        scope: 'process',
        setup: (context) => {
          context.contributeCapabilityGuard({
            id: 'z-guard',
            order: 5,
            evaluate: () => ({ allowed: true }),
          });
          context.contributeProjection({
            id: 'z-projection',
            order: 5,
            eventTypes: ['run:terminal'],
            project: () => {},
      ...
      makePlugin({
        ...
        setup: (context) => {
          context.contributeCapabilityGuard({
            id: 'a-guard-b',
            order: 1,
            evaluate: () => ({ allowed: false, reason: 'shell blocked' }),
          });
          context.contributeCapabilityGuard({
            id: 'a-guard-a',
            order: 1,
          ...
          context.contributeProjection({
            id: 'a-projection',
            order: 1,
            eventTypes: ['run:started', 'run:terminal'],
            project: () => {},
          });
        },
      }),
    ...
    const scopeA = await createProcessScope(buildPlugins());
    const scopeB = await createProcessScope(buildPlugins().reverse());
    const expectedGuards: Array<readonly [string, number, string]> = [
      ['a-guard-a', 1, 'a-plugin'],
      ['a-guard-b', 1, 'a-plugin'],
      ['z-guard', 5, 'z-plugin'],
    ];
    const expectedProjections: Array<readonly [string, number, string]> = [
      ['a-projection', 1, 'a-plugin'],
      ['z-projection', 5, 'z-plugin'],
    ];
    for (const scope of [scopeA, scopeB]) {
      expect(scope.listCapabilityGuards().map((guard) => [guard.id, guard.order, guard.pluginId] as const)).toEqual(expectedGuards);
      expect(scope.listProjections().map((projection) => [projection.id, projection.order, projection.pluginId] as const)).toEqual(expectedProjections);
      expect(scope.snapshot().capabilityGuards.map((guard) => guard.id)).toEqual(
        expectedGuards.map((entry) => entry[0]),
      );
      expect(scope.snapshot().projections.map((projection) => projection.id)).toEqual(
        expectedProjections.map((entry) => entry[0]),
      );
      await scope.dispose();
    }
  });

  test('defaults omitted projection orders to 0 for deterministic inventory', async () => {
    const buildPlugins = (): CapekPlugin<unknown>[] => [
      makePlugin({
        id: 'z-proj',
        scope: 'process',
        setup: (context) => {
          context.contributeProjection({ id: 'z-default', eventTypes: ['run:disposed'], project: () => {} });
          context.contributeProjection({ id: 'z-ordered', order: 10, eventTypes: ['run:disposed'], project: () => {} });
        },
      }),
      makePlugin({
        id: 'a-proj',
        scope: 'process',
        setup: (context) => {
          context.contributeProjection({ id: 'a-default', eventTypes: ['run:disposed'], project: () => {} });
          context.contributeProjection({ id: 'a-ordered', order: -5, eventTypes: ['run:disposed'], project: () => {} });
        },
      }),
    ];
    const scopeA = await createProcessScope(buildPlugins());
    ...
    for (const scope of [scopeA, scopeB]) {
      expect(scope.listProjections().map((projection) => [projection.id, projection.order] as const)).toEqual(expected);
      expect(scope.snapshot().projections.map((projection) => projection.order)).toEqual(
  ...
  test('lists registered guards as metadata and never invokes evaluate', async () => {
    const calls: string[] = [];
    const scope = await createProcessScope([
      makePlugin({
        id: 'guard-registrar',
        ...
        setup: (context) => {
          context.contributeCapabilityGuard({
            ...
            order: 30,
            evaluate: (request) => {
              calls.push(`evaluate:${request.capability.id}`);
              return { allowed: false, reason: 'read is blocked' };
            },
          });
          context.contributeCapabilityGuard({
            id: 'guard-write',
    ...
    expect(scope.listCapabilityGuards().map((guard) => guard.id)).toEqual(['guard-write', 'guard-read']);
    const writeGuard = scope.listCapabilityGuards().find((guard) => guard.id === 'guard-write');
    expect(writeGuard?.order).toBe(10);
    expect(writeGuard?.pluginId).toBe('guard-registrar');
    expect(writeGuard?.scopeKind).toBe('process');
    expect(calls).toEqual([]);
    await scope.dispose();
  });

  test('lists projections as metadata and never invokes project or rebuild', async () => {
    const calls: string[] = [];
    const scope = await createProcessScope([
      makePlugin({
        id: 'projection-registrar',
        scope: 'process',
        setup: (context) => {
          context.contributeProjection({
            id: 'telemetry-projection',
            eventTypes: ['run:terminal', 'run:disposed'],
            project: (event) => {
              calls.push(`project:${event.type}`);
            },
            rebuild: async () => {
              calls.push('rebuild');
            },
    ...
    const listed = scope.listProjections();
    expect(listed.map((projection) => projection.id)).toEqual(['telemetry-projection']);
    expect(listed[0]?.order).toBe(0);
    expect(listed[0]?.eventTypes).toEqual(['run:terminal', 'run:disposed']);
    expect(listed[0]?.pluginId).toBe('projection-registrar');
    expect(listed[0]?.scopeKind).toBe('process');
    expect(calls).toEqual([]);
    await scope.dispose();
  });

  test('JSON diagnostics expose guard and projection metadata without functions', async () => {
    const scope = await createProcessScope([
      makePlugin({
        id: 'meta-plugin',
        scope: 'process',
    ...
    const guard = scope.listCapabilityGuards().find((entry) => entry.id === 'json-guard');
    expect(guard).toEqual({ id: 'json-guard', order: 1, pluginId: 'meta-plugin', scopeKind: 'process' });
    const projection = scope.listProjections().find((entry) => entry.id === 'json-projection');
    ...
    function assertNoFunctions(value: unknown, path: string): void {
      if (typeof value === 'function') {
        throw new Error(`function leaked at ${path}`);
      }
      if (Array.isArray(value)) {
        value.forEach((entry, index) => assertNoFunctions(entry, `${path}[${index}]`));
    ...
    assertNoFunctions(scope.snapshot(), 'snapshot');
    const serialized = JSON.stringify(scope.snapshot());
    assertNoFunctions(JSON.parse(serialized) as unknown, 'serialized snapshot');
  ...
  test('duplicate guard and projection ids fail across the parent scope chain', async () => {
    const processScope = await createProcessScope([
      makePlugin({
        id: 'process-guard',
        scope: 'process',
    ...
    const guardErr = await expectFailure(
      createAgentScope(processScope, [
        makePlugin({
          id: 'agent-guard',
          scope: 'agent',
      ...
      ActivationError,
      "capability guard 'shared-guard' is already registered in the process scope",
    );
    expect(guardErr.cause).toBeInstanceOf(DuplicateContributionError);

    const projectionErr = await expectFailure(
      createAgentScope(processScope, [
        makePlugin({
          id: 'agent-projection',
          scope: 'agent',
      ...
      ActivationError,
      "projection 'shared-projection' is already registered in the process scope",
    );
    expect(projectionErr.cause).toBeInstanceOf(DuplicateContributionError);

    await processScope.dispose();
  });

  test('distinct parent guards and projections are inherited by child scopes', async () => {
    const processScope = await createProcessScope([
      makePlugin({
        id: 'process-guard',
        scope: 'process',
    ...
    const runScope = await createRunScope(agentScope, 'guard-run', []);
    expect(runScope.listCapabilityGuards().map((guard) => [guard.id, guard.scopeKind])).toEqual([
      ['process-guard', 'process'],
      ['agent-guard', 'agent'],
    ]);
    expect(runScope.listProjections().map((projection) => [projection.id, projection.scopeKind])).toEqual([
      ['process-projection', 'process'],
    ]);
    expect(runScope.snapshot().capabilityGuards.map((guard) => guard.id)).toEqual([
      'process-guard',
      'agent-guard',
    ]);
    await runScope.cancel('done').completion;
  ...
  test('rejects malformed guard and projection shapes before registration lands', async () => {
    async function expectMalformedSetup(plugin: CapekPlugin<unknown>, fragment: string): Promise<void> {
      const err = await expectFailure(createProcessScope([plugin]), ActivationError, 'setup failed');
      expect(err.cause).toBeInstanceOf(MalformedPluginError);
      expect(String(err.cause)).toContain(fragment);
    }

    const badGuardEvaluate = makePlugin({
      id: 'bad-guard-evaluate',
      scope: 'process',
      setup: (context) => {
        context.contributeCapabilityGuard(
          { id: 'guard', order: 1 } as unknown as CapabilityGuardContribution,
        );
      },
    });
    await expectMalformedSetup(badGuardEvaluate, "capability guard 'guard' must provide an evaluate function");

    const badGuardOrder = makePlugin({
      id: 'bad-guard-order',
      scope: 'process',
      setup: (context) => {
        context.contributeCapabilityGuard({
    ...
    await expectMalformedSetup(badGuardOrder, "capability guard contribution 'guard' must have a finite order");

    const badProjectionId = makePlugin({
      id: 'bad-projection-id',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: '',
    ...
    await expectMalformedSetup(badProjectionId, 'projection contribution id must be a non-empty string');

    const badProjectionEventTypes = makePlugin({
      id: 'bad-projection-event-types',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: 'projection',
          eventTypes: 'run:terminal' as unknown as string[],
          project: () => {},
        });
      },
    });
    await expectMalformedSetup(
      badProjectionEventTypes,
      "projection contribution 'projection' eventTypes must be an array of non-empty strings",
    );

    const emptyProjectionEventTypes = makePlugin({
      id: 'empty-projection-event-types',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: 'projection',
    ...
    await expectMalformedSetup(
      emptyProjectionEventTypes,
      "projection contribution 'projection' must declare at least one event type",
    );

    const badProjectionCallback = makePlugin({
      id: 'bad-projection-callback',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: 'projection',
          eventTypes: ['run:terminal'],
        } as unknown as ProjectionContribution);
      },
    });
    await expectMalformedSetup(badProjectionCallback, "projection 'projection' must provide a project function");

    const badProjectionRebuild = makePlugin({
      id: 'bad-projection-rebuild',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: 'projection',
          eventTypes: ['run:terminal'],
          project: () => {},
          rebuild: 'rebuild' as unknown as () => void,
        });
      },
    });
    await expectMalformedSetup(badProjectionRebuild, "projection 'projection' rebuild must be a function when present");

    const badProjectionOrder = makePlugin({
      id: 'bad-projection-order',
      scope: 'process',
      setup: (context) => {
        context.contributeProjection({
          id: 'projection',
    ...
    await expectMalformedSetup(
      badProjectionOrder,
      "projection contribution 'projection' must have a finite order when present",
    );
  });
});
