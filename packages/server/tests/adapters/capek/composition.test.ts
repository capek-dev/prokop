...
import { isWithin, parseImports, resolveLocalSpecifier } from '../../helpers/import-scan';
...
describe('Čapek composition root', () => {
  ...
  test('imports only the focused capek adapters and no domain implementations', () => {
    ...
    for (const imp of imports) {
      ...
      if (resolved !== null) {
        expect(isWithin(resolved, adaptersDir)).toBe(true);
      }
    }
  });
...
describe('C4 coding bundle in the Jean2 composition', () => {
  ...
  test('exposes the exact standard contributed coding inventory without a scoped resolver', async () => {
    ...
    expect(tools.map((tool) => tool.definition.name)).toEqual(STANDARD_CODING_TOOL_NAMES);
